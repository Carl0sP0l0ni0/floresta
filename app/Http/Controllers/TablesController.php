...
class TablesController extends Controller
{
    ...
    public function new(Request $request): RedirectResponse
    {
        ...
        return redirect()->back()->with( 'Mesa #' . $table->id . ' creado!');
    }
    ...
    public function edit(Table $table, Request $request): RedirectResponse
    {
        ...
        return redirect()->back()->with( 'Capacidade da mesa #' . $table->id . ' modificado');
    }
    ...
    public function delete(Table $table): RedirectResponse
    {
        ...
        return redirect()->back()->with('borrado!');
    }
    ...
    public function busy(Table $table): RedirectResponse
    {
        ...
        if ($table->busy && $table->product_count > 0){
            return redirect()->back()
                            ->withErrors('¡La mesa con productos no puede quedar libre sin pago!');
        }
        ...
        return redirect()->back()->with( 'Mesa #' . $table->id . ' ' . $status . ' éxito!');
    }

    /**
     * Adicionar productos a la mesa
     *
     * @param Table $table
     * @param Request $request
     * @return RedirectResponse
    ...
    {
        $productsStd = json_decode($request->input('products-add-json'), false);
        $products = (array) $productsStd;
        ...
        foreach ($products as $key => $quantity) {
            ...
            foreach ($allProducts as $product) {
                if ($product->id === $key) {
                    ...
                    $value = $value + ($product->value * $quantity);
                    break;
                }
        ...
        return redirect()->back()->with('Produto(s) adicionado(s) con exito!');
    }
