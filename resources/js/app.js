...
$('.btn-manage-table').on('click', function () {
    ...
    let tableIdToString = tableId.toString()
    let newDetails = apiDetails.replace(/\/0\//, `/${tableIdToString}/`);
    $.get(newDetails)
        .done((data) => {
...
function getDetails(typeData, itemId) {
    ...
    let newDetails = apiDetails.replace(/\/0\//, `/${itemId}/`);
    $.get(newDetails)
        .done((data) => {
            load.addClass('d-none')
...
function generateDetailsHTML(items, tableId = false, links = true) {
    ...
    if (typeof items[0] === 'undefined') HTML = '<li class="list-group-item text-muted text-center">No hay productos en mesa!</li>'
    else {
...
function generateAddHTML(name, valueUnitary = null, quantity = null, id = null) {
    ...
    if (name === null) HTML = '<li class="list-group-item text-muted text-center">No hay productos que adicionar!</li>'
    else {
        HTML = `<li class="list-group-item d-flex justify-content-between lh-sm" id="products-add-${id}">
                    <div>
                        ...
                        <small class="text-muted">${"S/. "+(valueUnitary)}</small>
                    </div>
                    <div>
                        <span class="text-muted me-2" id="products-add-${id}-value" data-value=${value}>${"S/. "+(value)}</span>
...
function generatePayHTML() {
    ...
    for (let i = 1; i <= peoplesPay; i++) {
        HTML += `<div class="mt-3">
                    <label for="method-${i}" class="form-label">Cliente ${i} Pagado</label>
