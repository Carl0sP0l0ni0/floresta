...
@section('content')
    <main class="container-fluid">
        <div class="p-3 pb-2 mb-3 border-bottom d-flex justify-content-between">
            <h1 class="h2">Resumen de la tabla #{{ $tableId }}</h1>
        </div>
        @if ($errors->any())
                ...
                <div class="my-auto p-3">
                    ...
                    <div class="mt-3">
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item">Pago por {{ count($methods) }} persona(s)</li>
                            <li class="list-group-item">Valor de {{ $value }}/persona</li>
                            @for($i = 1; $i <= count($methods); $i++)
                                <li class="list-group-item">O cliente {{ $i }} pagado en {{ $methods[$i-1] }}</li>
                            @endfor
                        </ul>
                    </div>
                </div>
